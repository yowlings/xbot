...
bool Sensors::deserialise(ecl::PushAndPop<unsigned char>& byteStream) {
  ...
  //    std::cout <<
  //    "bytestream.size:"<<byteStream.size()<<std::endl<<"xbot_node:
  //    xbot_default: deserialise failed. not enough byte stream." << std::endl;
  //    return false;
  //  }
  ...
  if (header_id != Header::Sensors) {
    std::cout << "header_id is wrong. header_id:" << (unsigned int)header_id
              << std::endl;

 ...
 * reading thread (aye, convoluted - apologies for the multiple robot and
 * multiple
...
/**
 * @brief Use the current sensor data (encoders and gyro) to calculate an update
 * for the odometry.
 *
 * This fuses current sensor data with the last updated odometry state to
 * produce the new
 * odometry state. This will be usually done in the slot callback to the
 * stream_data signal.
 *
 * It is important that this is called every time a data packet is received from
 * the robot.
 *
 ...
 * to do so above.
 *
 * @param command : prepared command template (see Command's static member
 * functions).
 */
